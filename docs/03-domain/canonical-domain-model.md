...
Tài liệu này xác định ngôn ngữ domain và quyền sở hữu khái niệm chuẩn cho MVP. Nó không tạo database schema, API hay implementation. Mọi public contract phải dùng tên/ý nghĩa ở đây; chi tiết wire/DDL thuộc `contracts/` và `docs/04-data/` sau khi được phê duyệt.
