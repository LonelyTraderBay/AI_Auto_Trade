...
| FR-FE-003 | Incident/reconciliation/audit views | Xem incident (severity/status), audit events bất biến, trạng thái reconciliation | (a) Incident list filter được theo severity CRITICAL/HIGH/MEDIUM/LOW và status OPEN/MITIGATING/RESOLVED/CLOSED (openapi.yaml schema Incident) — lưu ý: GET /incidents v1.1 chỉ có cursor/limit, chưa có filter param; filter tạm thời là client-side trên trang đã tải, filter server-side là GAP 9 tại FE-SCREEN-001 §4; (b) audit event hiển thị actor/action/before_hash/after_hash (schema AuditEvent); (c) cursor pagination; (d) đây là deliverable Phase 5 "incident/audit/reconciliation/deployment view" (master §14 Phase 5) | GET /incidents, /audit-events, /deployments/{deployment_id} (openapi.yaml) |
...
|---|---|---|---|---|
| 2026-08-02 | 0.1.1 | Technical Operator | Pending | Audit toàn diện: FR-FE-003(a) ghi rõ filter /incidents là client-side tạm thời (GAP 9 mới tại FE-SCREEN-001 §4); FR-FE-007 sửa cách đếm "12 route /ai/*" thành 11 path/12 operation cho nhất quán với "28 routes" |
