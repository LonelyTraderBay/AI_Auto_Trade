...
|---|---|---|
| Governance/product/architecture | docs/00-governance, docs/01-product, docs/02-architecture | Tên kebab-case, Markdown, header bắt buộc. |
| ADR | docs/adr | ADR-NNNN-<slug>.md; số không tái sử dụng. |
| Contract | contracts/ | File versioned; canonical path ghi trong contract registry. |
...
| Gate/task evidence | docs/evidence/gates hoặc docs/evidence/tasks | Có hash/path, runner, UTC time và liên kết Task/Gate ID. |
| Generated artifact | generated/ hoặc đường dẫn task nêu rõ | Có banner/source link; không sửa tay. |

DOCS_INDEX.md là registry điều hướng của artifact pack và phải được cập nhật trước gate Phase 0.0. Registry không thay thế metadata trong từng artifact.
